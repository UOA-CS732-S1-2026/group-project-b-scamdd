...
const TH =
  'px-4 py-3 text-xs font-semibold uppercase tracking-wider text-[var(--c-text-2)] text-left whitespace-nowrap';
...
export default function Transactions() {
  ...
  const filteredTransactions = useMemo(() => {
    ...
    return [...list].sort((a, b) => {
      switch (sortBy) {
        ...
        case 'happiest':
          return (
            (MOOD_DISPLAY[b.mood ?? '']?.value ?? 0) - (MOOD_DISPLAY[a.mood ?? '']?.value ?? 0)
          );
        case 'regretful':
          return (
            (MOOD_DISPLAY[a.mood ?? '']?.value ?? 99) - (MOOD_DISPLAY[b.mood ?? '']?.value ?? 99)
          );
      }
    });
  }, [
  ...
  const stats = useMemo(() => {
    ...
    const biggest = expenses.reduce<Transaction | null>(
      (b, t) => (!b || Math.abs(t.amount) > Math.abs(b.amount) ? t : b),
      null,
    );
    const moodList = filteredTransactions.filter(
      (t) => t.essential === false && t.mood && MOOD_DISPLAY[t.mood],
    );
    const moodTotalSpent = moodList.reduce((s, t) => s + Math.abs(t.amount), 0);
    const moodAvg =
      moodTotalSpent > 0
        ? moodList.reduce((s, t) => s + MOOD_DISPLAY[t.mood!].value * Math.abs(t.amount), 0) /
          moodTotalSpent
        : 0;
    const moodEmoji =
      moodAvg >= 4.5
  ...
  const dailySpend = useMemo(() => {
    ...
    if (expenses.length === 0)
      return { bars: [] as { day: number; total: number }[], max: 0, monthLabel: '' };
  ...
  const lastLogged = useMemo(() => {
    ...
    const latest = transactions.reduce(
      (b, t) => (new Date(t.date) > new Date(b.date) ? t : b),
      transactions[0],
    );
          ...
          {dailySpend.bars.length === 0 ? (
            ...
            <>
              <div className="text-xs text-[var(--c-text-2)] mb-4">
                {dailySpend.monthLabel} · {fmt(dailySpend.bars.reduce((s, b) => s + b.total, 0))}{' '}
                spent
              </div>
                ...
                {dailySpend.bars.map((b) => (
                  <div key={b.day} className="flex-1 flex justify-center">
                    {(b.day === 1 || b.day % 5 === 0 || b.day === dailySpend.bars.length) && (
                      <span className="text-[10px] text-[var(--c-text-2)] leading-none">
                        {b.day}
                      </span>
                    )}
                  </div>
        ...
        <div className="flex flex-col gap-6">
          <aside className="p-5 rounded-3xl border border-[rgba(109,109,109,0.8)] bg-[var(--c-card)]">
            ...
            >
              ...
              <span
                aria-hidden
                className={`transition-transform text-[var(--c-text-2)] ${filtersOpen ? 'rotate-180' : ''}`}
              >
                ▾
              </span>
            </button>
            ...
            {filtersOpen && (
              <>
                ...
                <div className="relative mb-4">
                  <svg
                    className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--c-text-2)] pointer-events-none"
                    width="15"
                    height="15"
                    viewBox="0 0 15 15"
                    fill="none"
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-1">
                  <FilterGroup label="SORT BY">
                    {(
                      ['recent', 'largest', 'smallest', 'happiest', 'regretful'] as SortOption[]
                    ).map((s) => (
                  ...
                  <FilterGroup label="MOOD">
                    <FilterChip active={filterMood === 'all'} onClick={() => setFilterMood('all')}>
                      All
                    </FilterChip>
                    {Object.entries(MOOD_DISPLAY).map(([key, m]) => (
                      <FilterChip
                        key={key}
                        active={filterMood === key}
                        onClick={() => setFilterMood(key)}
                      >
                        <span title={m.label} aria-label={m.label} className="text-base">
                          {m.emoji}
                        </span>
                      </FilterChip>
                    ))}
                          ...
                          <tbody>
                            {pagedTransactions.map((t) => (
                              ...
                              >
                                ...
                                {/* Title */}
                                <td
                                  className={`${TD} font-medium text-[var(--c-text)] max-w-[180px]`}
                                >
                                ...
                                {/* Amount */}
                                <td
                                  className={`${TD} text-right font-semibold whitespace-nowrap ${
                                    t.type === 'income'
                                      ? 'text-[var(--c-income)]'
                                      : 'text-[var(--c-expense)]'
                                  }`}
                        ...
                        {Array.from({ length: totalPages }, (_, i) => i + 1)
                          ...
                          .map((p, i) =>
                            p === '...' ? (
                              <span
                                key={`dots-${i}`}
                                className="px-2 text-xs text-[var(--c-text-2)]"
                              >
                                …
                              </span>
                            ) : (
...
function FilterGroup({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="mb-4">
      <div className="text-[11px] font-semibold tracking-wider text-[var(--c-text-2)] mb-2">
        {label}
      </div>
