...
function DonutChart({ slices }: { slices: { value: number; color: string }[] }) {
  const cx = 60, cy = 60, r = 42, strokeWidth = 20;
...
export default function DashboardPage() {
  ...
  if (viewPeriod === 'daily') {
    const N = 24;
    cumulativePoints = Array.from({ length: N }, (_, h) =>
      expenses.filter((t) => new Date(t.date).getHours() <= h)
    ...
    const N = 7;
    cumulativePoints = Array.from({ length: N }, (_, i) =>
      expenses.filter((t) => {
    ...
    cumulativePoints = Array.from({ length: N }, (_, i) => {
      const dayNum = i + 1;
      return expenses.filter((t) => new Date(t.date).getDate() <= dayNum)
                     .reduce((s, t) => s + Math.abs(t.amount), 0);
    });
    xTicks = [1, 5, 10, 15, 20, 25, N]
      .filter((d, i, a) => a.indexOf(d) === i && d <= N)
      .map((d) => ({ idx: d - 1, label: String(d) }));
  } else {
    const N = 12;
    cumulativePoints = Array.from({ length: N }, (_, m) =>
      expenses.filter((t) => new Date(t.date).getMonth() <= m)
              .reduce((s, t) => s + Math.abs(t.amount), 0),
    );
  ...
  const breakdownRows: { label: string; slices: { label: string; value: number; color: string }[] }[] = [
  ...
  return (
    <div className="min-h-screen bg-[var(--c-bg)] text-[var(--c-text)]">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex items-center gap-1.5">
                <button
                  ...
                  className="w-7 h-7 flex items-center justify-center rounded-lg border border-[var(--c-border)] text-[var(--c-text-2)] hover:opacity-80 text-base leading-none"
                >‹</button>
                <span className="text-sm font-medium text-[var(--c-text)] min-w-[160px] text-center">
                  {label}
                </span>
                <button
                  onClick={() => setPeriodAnchor(shiftAnchor(viewPeriod, periodAnchor, 1))}
                  disabled={isCurrent}
                  className="w-7 h-7 flex items-center justify-center rounded-lg border border-[var(--c-border)] text-[var(--c-text-2)] hover:opacity-80 disabled:opacity-25 disabled:cursor-not-allowed text-base leading-none"
                >›</button>
              </div>
            </div>
            <button
        ...
        {/* ── Row 1: 2×2 stat cards (left) + cumulative chart (right) ── */}
        <div className="grid grid-cols-[2fr_3fr] gap-6 mb-6">
          ...
          <div className={panelClass}>
            <div className="flex justify-between items-start mb-3">
              ...
              <button
                onClick={() => navigate('/transactions')}
                className="text-sm font-medium hover:opacity-80 flex-shrink-0 text-[var(--c-accent)]"
              >
                View more →
              </button>
            </div>
            ...
            <svg width="100%" viewBox={`0 0 ${SVG_W} ${SVG_H}`} style={{ display: 'block', overflow: 'visible' }}>
              ...
              {budgetLineY !== null && (
                <>
                  <line x1={PAD_L} y1={budgetLineY} x2={PAD_L + PLOT_W} y2={budgetLineY}
                    stroke="var(--c-text-2)" strokeWidth="1.5" strokeDasharray="5 4" />
              ...
              {spendPts.length > 1 && (
                <polyline points={spendPts.join(' ')} fill="none" stroke="var(--c-accent)"
              ...
              {xTicks.map(({ idx, label: xl }) => (
                <text key={idx} x={cxFn(idx)} y={SVG_H - 3} textAnchor="middle" fontSize="9" fill="var(--c-text-2)">
                  {xl}
                </text>
              ))}
            </svg>
        ...
        {/* ── Row 2: mood (left) + category donut (right) ── */}
        <div className="grid grid-cols-2 gap-6 mb-6">
          <div className={`${panelClass} flex flex-col`}>
            <div className="flex justify-between items-center mb-1">
              ...
              <button onClick={() => navigate('/transactions')} className="text-sm font-medium hover:opacity-80 text-[var(--c-accent)]">
                View more →
              </button>
            </div>
                ...
                <div className="flex-1 relative" style={{ minHeight: '120px' }}>
                  ...
                  <div className="absolute inset-0 flex items-end gap-2 pb-px">
                    {MOOD_LABELS.map((lbl, i) => {
                      ...
                      const heightPct = (amount / maxMood) * 100;
                      return (
                        <div key={lbl} className="flex-1 h-full flex items-end">
                          <div
                            title={`$${amount.toFixed(2)}`}
                            style={{ backgroundColor: MOOD_COLORS[i], height: amount > 0 ? `${Math.max(heightPct, 2)}%` : '0%' }}
        ...
        {/* ── Row 3: recent transactions (left tall) + breakdown bars & leaderboard (right) ── */}
        <div className="grid grid-cols-2 gap-6">

          {/* Recent transactions — scrollable, fills height */}
          <div className={`${panelClass} flex flex-col`}>
            <div className="flex justify-between items-center mb-1">
            ...
            <div className="flex-1 overflow-y-auto min-h-0" style={{ maxHeight: '520px' }}>
              {recentAll.length === 0 ? (
                ...
                <div className="space-y-0.5">
                  {recentAll.map((t) => (
                    <div key={t._id} className="flex justify-between items-center py-2.5 px-3 rounded-xl hover:bg-[var(--c-surface)] transition-colors">
                      ...
                      <div className={`text-sm font-semibold flex-shrink-0 ml-4 ${
          ...
          {/* Right: breakdown bars + leaderboard stacked */}
          <div className="flex flex-col gap-6">
            ...
            <div className={panelClass}>
              <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={() => navigate('/friends')} className="text-sm font-medium hover:opacity-80 text-[var(--c-accent)]">
                  Manage →
                </button>
              </div>
              {friends.length === 0 ? (
                ...
                <div className="flex flex-col gap-2">
                  {leaderboard.map((entry, idx) => (
                    <div
                      key={entry.id}
                      className={`flex items-center gap-3 py-2 px-2 rounded-xl ${entry.isMe ? 'bg-[var(--c-surface)]' : ''}`}
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-sm font-medium truncate text-[var(--c-text)]">
                          {entry.name}
                          {entry.isMe && <span className="text-xs text-[var(--c-text-2)] ml-1">(you)</span>}
                        </div>
                      </div>
