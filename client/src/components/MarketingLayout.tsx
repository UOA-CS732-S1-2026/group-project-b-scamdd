...
export default function MarketingLayout({ children }: { children: ReactNode }) {
  ...
  const ghostBtn =
    'px-3 sm:px-4 py-1.5 rounded-[20px] text-xs sm:text-sm font-medium text-[var(--c-text)] bg-transparent hover:bg-[var(--c-nav-active)] transition-colors cursor-pointer';
  const primaryBtn =
    'px-3 sm:px-4 py-1.5 rounded-[20px] text-xs sm:text-sm font-medium border border-[var(--c-text)] bg-[var(--c-text)] text-[var(--c-bg)] hover:opacity-90 transition-opacity cursor-pointer';
  const iconBtn =
      ...
      <header className="sticky top-0 z-40 backdrop-blur bg-[color-mix(in_srgb,var(--c-bg)_85%,transparent)] border-b border-[var(--c-border)]">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-2">
          ...
          <nav className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
            <button type="button" onClick={toggle} className={iconBtn} aria-label="Toggle theme">
